...
document.querySelector('.carousel').addEventListener('click', function (evt){
    ...
    if (target.classList.contains('control')) {
        if (target.classList.contains('next')) {
            ...
            currentImg -= 1;
            if(currentImg === -1) ////currentimg is past first element
            {
...
{
    //Creats a MouseEvent object to simulate a 'click' that moves up the DOM
    var click = new MouseEvent('click',
     {
         'bubbles':true
    });

    //for a 'keycode' event, there will be a value in the .keycode property
    switch(evt.keyCode)
    {
